        ...
        if num_completed_scans > 0:
            if request.method == 'POST':
                ...
                if viz_form.is_valid():
                    ...
                    # EYE HEIGHT NEEDS TO EVENTUALLY BE CHANGED TO EYE POS
                    print("about to show plot")
                    [graph, graph_str] = viz_overhead(
                        json.loads(completed_scan.nvp_xs), json.loads(completed_scan.nvp_ys), (scan.F_m + ((scan.E_m - scan.F_m) / 2) + 1.2), ((scan.B_m - scan.A_m) / 2), vrus_selected)
