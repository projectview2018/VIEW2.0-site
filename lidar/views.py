...
def visualization(request, scan_id):
    ...
    if scan.scan_status == "modified":
        ...
        thread.start()
        return render(request, 'lidar/visualization.html', {'VisualizationForm': viz_form, 'scan_id': scan_id, 'make': make, 'model': model, 'year': year, 'date': scan.scan_added, 'graph': None, 'graph_str': None, 'vrus_selected': vrus_selected, "loading": True})
    ...
    if scan.scan_status == "processed":  # scan should be processed and status updated as such
        ...
        if num_completed_scans > 0:
            ...
            return render(request, 'lidar/visualization.html', {'VisualizationForm': viz_form, 'scan_id': scan_id, 'make': make, 'model': model, 'year': year, 'date': scan.scan_added, 'graph': None, 'graph_str1': None, 'graph_str2': None, 'vrus_selected': vrus_selected, "loading": False})
